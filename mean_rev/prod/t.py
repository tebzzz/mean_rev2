...
def get_quantities(tickers, global_data, data, position_ratios):
    ...
    for t in tickers: 
        prices[t] = global_data[f'{t}'].iloc[0]
...
def check_tables(data, tickers):
    ...
    position_ratios = create_model(tickers)  #create a function to create teh intial model (and maybe later ones)                              
...
def run_script_crypto(tickers, data, current_state=current_state):
    ...
    new_quantities = get_quantities(formatted_tickers, global_data, data, position_ratios) #the quantities do not seem correct
