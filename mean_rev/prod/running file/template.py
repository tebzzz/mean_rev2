...
def set_positions_database(ticker1, ticker2, ticker1_quantity, ticker2_quantity, prior_state, current_pos, use_crypto=False, stop=False): 
    global internal_id 
...
def decide_positions(z, moving_avg, upper_band, lower_band, current_pos='short'):
    global reset_flag, trade_status
    if current_pos == None:
        reset_flag = False
        if z > upper_band:
            trade_status = 'open'
...
def run_script_crypto(ticker1, ticker2, current_state=current_state, last_pos=last_pos):
    ...
    while True: 
        ...
        current_state = decide_positions(curr_zscore, curr_moving_avg, curr_upper_band, curr_lower_band, current_state)
        ...
        if (prior_state != current_state) and reset_flag == False:
            if prior_state == None and entry_signal_flag == False: 
                ...
                set_positions_database(ticker1, ticker2, new_quantity_a, new_quantity_b, prior_state, current_state,use_crypto=True)
