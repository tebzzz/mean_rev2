...
def decide_positions(z, moving_avg, upper_band, lower_band, current_pos='short'):
...
def get_spread(global_data, hedge_ratio, prices_a=None, prices_b=None): 
...
def get_data(global_data, hedge_ratio, spread=None):

    if spread == None:
        spread = get_spread(global_data, hedge_ratio)
...
def check_websockets(data, ticker1, ticker2, bar_size, ohlcv=False): 
    ...
    wait_for_tables(data, ticker1, ticker2, bar_size)
...
def check_tables(data, ticker1, ticker2):
    ...
    spread = get_spread(global_data, hedge_ratio, prices_a, prices_b)
...
def update_global_data_crypto(ticker1, ticker2, global_data, data, hedge_ratio): 

    duration_lookback = data['duration_lookback']
    bar_size_lookback = data['bar_size_lookback']
    ...
    spread = get_spread(global_data, hedge_ratio, prices_a = data1_close, prices_b = data2_close)
...
def run_script_crypto(ticker1, ticker2, data, current_state=current_state, last_pos=last_pos):
    ...
    check_websockets(data, ticker1_formatted, ticker2_formatted, bar_size_lookback, ohlcv=ohlcv)
    
    global_data = update_global_data_crypto(ticker1, ticker2, global_data, data, hedge_ratio) #this will trip an error, need to fix the global data
    ...
    while True: 
        global_data = update_global_data_crypto(ticker1, ticker2, global_data)
        ...
        if use_stoploss == True: 
            internal_id = check_exit_conditions(global_data, ticker1, ticker2, new_quantity_a, new_quantity_b,current_state, data, percent_stop=True)

        prior_state = current_state

        current_state, reset_flag, trade_status = decide_positions(curr_zscore, curr_moving_avg, curr_upper_band, curr_lower_band,current_state)
...
stoploss_break=break_on_loss, band_vol_exp=bollinger_vol_exp, bol_band_type=bollinger_type):
    ...
    data['duration_lookback'] = lookback_input*5 #duration_lookback = lookback_input*5
    data['bar_size_lookback'] = bar_size_lookback_input #bar_size_lookback = bar_size_lookback_input
